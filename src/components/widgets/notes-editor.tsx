...
export function NotesEditor() {
  const [note, setNote] = useState(() => {
    const savedNote = localStorage.getItem('notes-content')
    return savedNote || ''
  })
  ...
  useEffect(() => {
    if (!isEditing) {
      localStorage.setItem('notes-content', note)
    }
  }, [note, isEditing])
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-6">
      ...
      {isEditing ? (
        ...
        <div 
          className="w-full min-h-[200px] overflow-auto rounded-md border border-input bg-background px-3 py-2 text-sm prose dark:prose-invert prose-sm max-w-none resize-y"
          style={{ 
            maxHeight: '80vh',
            cursor: 'ns-resize'
          }}
